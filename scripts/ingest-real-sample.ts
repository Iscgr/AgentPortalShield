...
function parseArgs() {
  ...
  return { dryRun: args.includes('--dry-run'), apply: args.includes('--apply'), limit: (()=>{ const i=args.indexOf('--limit'); return i>=0?Number(args[i+1]):undefined; })() };
}
...
async function main() {
  ...
  for (let gIndex=0; gIndex<groups.length; gIndex++) {
    ...
    if (apply) {
      ...
      if (exists) { skippedInvoices++; continue; }
