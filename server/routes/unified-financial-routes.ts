...
/**
 * ✅ PERFORMANCE OPTIMIZATION: Batch endpoint for multiple representatives
 * Replaces individual API calls to prevent Query Storm
 */
router.post('/representatives/batch', requireAuth, async (req, res) => {
  try {
    const { representativeIds, batchSize = 20 } = req.body;
    
    if (!Array.isArray(representativeIds) || representativeIds.length === 0) {
      return res.status(400).json({
        success: false,
        error: "شناسه‌های نمایندگان الزامی است",
        details: "representativeIds must be a non-empty array"
      });
    }

    console.log(`🚀 PERFORMANCE: Processing batch financial calculation for ${representativeIds.length} representatives`);
    const startTime = performance.now();

    // ✅ BATCH SIZE LIMITING: Prevent overwhelming server
    const maxBatchSize = 50;
    const limitedIds = representativeIds.slice(0, maxBatchSize);
    
    if (representativeIds.length > maxBatchSize) {
      console.log(`⚠️ PERFORMANCE: Batch size limited from ${representativeIds.length} to ${maxBatchSize}`);
    }

    // ✅ SMART BATCHING: Use existing calculateAllRepresentatives for efficiency
    const engine = new UnifiedFinancialEngine(null);
    const allFinancialData = await engine.calculateAllRepresentatives();
    
    // ✅ FILTER RESULTS: Only return requested representatives
    const requestedData = allFinancialData.filter(data => 
      limitedIds.includes(data.id)
    );

    // ✅ MISSING IDS HANDLING: Fill in missing representatives with minimal data
    const missingIds = limitedIds.filter(id => 
      !requestedData.find(data => data.id === id)
    );

    if (missingIds.length > 0) {
      console.log(`⚠️ PERFORMANCE: ${missingIds.length} representatives not found in batch calculation`);
      
      // Add placeholder data for missing representatives
      for (const missingId of missingIds) {
        requestedData.push({
          id: missingId,
          name: `نماینده ${missingId}`,
          code: `REP-${missingId}`,
          totalSales: 0,
          totalPaid: 0,
          totalDebt: 0,
    ...
    console.log(`✅ PERFORMANCE: Batch calculation completed in ${Math.round(processingTime)}ms for ${requestedData.length} representatives`);
    console.log(`🎯 PERFORMANCE: Individual queries avoided: ${limitedIds.length} → 1 batch query`);

    res.json({
      success: true,
      data: requestedData,
      meta: {
        source: "UNIFIED FINANCIAL ENGINE BATCH v1.0",
        timestamp: new Date().toISOString(),
        requested: representativeIds.length,
        processed: limitedIds.length,
        returned: requestedData.length,
        processingTime: Math.round(processingTime),
        efficiency: `${limitedIds.length}x faster than individual queries`
      }
    });

  } catch (error) {
    console.error('❌ PERFORMANCE: Batch financial calculation error:', error);
    res.status(500).json({
      success: false,
